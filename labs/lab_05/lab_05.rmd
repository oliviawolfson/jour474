...
```{r}
emergency_calls <- read_csv("tabula-Baltimore-County.csv", col_names = FALSE) |> clean_names() |>
 rename(date= x1, time= x2, case_number= x3, type= x4, location =x5) |>
  mutate(datec = mdy(date))
...
A1. 07/14/22 and 10/04/22 had the most overdose calls at 23. It could be possible that there were days with no overdose calls, but it also could be that the call made wasn't originally categorized as an overdose.
...
A3.4540 Silver Spring Rd has the most calls. This just seems like an apartment building and I don't know much about the Silver Spring area but I wasn't really expecting this. I wouldn't say there is anything apparently obvious in the original data that would make question the count especially since they're all pretty grouped together.
...
A4. I think a good story idea would be why one apartment complex has the most overdose calls. Why has this happened 36 times in one place?
