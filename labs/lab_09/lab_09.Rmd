...
**A1.** <https://datawrapper.dwcdn.net/VLt89/1/> I did two horizontal
bars next to eachother because I thought it was the best to visually
understand how much Hogan outperformed Cox

```{r}
md_gov_county <- read_csv("data/md_gov_county.csv") |> clean_names()
md_gov_county_18 <- read_csv("data/md_gov_county_18.csv") |> clean_names()
clean_names(md_gov_county) 
clean_names(md_gov_county_18)

all_county_votes <- md_gov_county |> inner_join(md_gov_county_18, by=c('geoid', 'county'))

all_county_votes <- all_county_votes |> mutate(
  total_votes_22= cox + moore + lashar + wallace + harding + sefcik + write_ins.x, cox_pct= cox/(total_votes_22)*100, total_votes_18 = hogan + jealous + quinn + schlakman + write_ins.y, hogan_pct = hogan/(total_votes_18)*100
)
write_csv(all_county_votes, 'data/all_county_votes.csv')
```
...
<https://datawrapper.dwcdn.net/al5v3/1/> The story is how dramatic the
methods changed in 2020 which is likely due to the pandemic

**A2.**

```{r}
md_votes_by_type <- read_csv("data/md_votes_by_type.csv") |> clean_names()
md_votes_by_type <- md_votes_by_type |> mutate( pct_mail= mail/(total)*100, pct_election_day= election_day/(total)*100, pct_early= early/(total)*100)
...
**A3.** <https://datawrapper.dwcdn.net/pxoEE/1/>
...
montgomery_2022_overdoses <- montgomery_2022_overdoses |>  
  group_by(zip)|>
  summarise(count_zip = n())|>
mutate((zip_pct=count_zip/sum(count))*100)
